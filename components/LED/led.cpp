...
std::array<bool, LEDC_TIMER_MAX> LED::timerAvailability =
    init_array_true<LEDC_TIMER_MAX>();

LED::LED(gpio_num_t aPin) : mPin(aPin), mBrightness(0) { initialize(); }
...
void LED::initialize() {
  ...
  ledc_channel_config_t ledc_channel = {.gpio_num = mPin,
...
ledc_channel_t LED::getAvailableChannel() {
  for (int i = 0; i < channelAvailability.size(); i++) {
    // ESP_LOGI(LOG_TAG, "Channel %d is %s", i, channelAvailability[i] ?
    // "available" : "in use");
...
ledc_timer_t LED::getAvailableTimer() {
  for (int i = 0; i < timerAvailability.size(); i++) {
    // ESP_LOGI(LOG_TAG, "Timer %d is %s", i, timerAvailability[i] ? "available"
    // : "in use");
