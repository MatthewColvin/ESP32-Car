...
public:
   ...
   */
  inline void mSetReceiveHandler(RxHandlerTy aReceiveHandler) {
    mDataReceivedHandler = aReceiveHandler;
  };
...
private:
  static constexpr auto IR_RESOLUTION_HZ =
      1000000; // 1MHz resolution, 1 tick = 1us;
  ...
  static bool onReceiveImpl(rmt_channel_handle_t rx_chan,
                            const rmt_rx_done_event_data_t *edata,
                            void *user_ctx);
  bool onReceive(const rmt_rx_done_event_data_t *edata);

  TaskHandle_t
      mReceiveProccess; // Task Handle for Receiving incoming transmissions
  void receiveTask();   // Task running on mReceiveProccess
  static void receiveTaskImpl(
      void *aThis); // Wrapper for Task so it can be used with FreeRtos
  ...
  RxHandlerTy mDataReceivedHandler =
      nullptr; // Callback for user of class to handle parsed data from the
               // Queue
  ...
  static bool onSendDoneImpl(rmt_channel_handle_t rx_chan,
