...
Transceiver::Transceiver(gpio_num_t receivePin, gpio_num_t sendPin)
    : mRxPin(receivePin), mTxPin(sendPin) {
  // Tx Configs
...
bool Transceiver::onReceiveImpl(rmt_channel_handle_t rx_chan,
...
bool Transceiver::onSendDoneImpl(rmt_channel_handle_t rx_chan,
...
void Transceiver::receiveTask() {
  ...
  receiveCfg.signal_range_min_ns =
      1250; // the shortest duration for NEC signal is 560us, 1250ns < 560us,
            // valid signal won't be treated as noise
  receiveCfg.signal_range_max_ns =
      12000000; // the longest duration for NEC signal is 9000us, 12000000ns >
                // 9000us, the receive won't stop early
  ...
  while (true) {
    ...
    if (xQueueReceive(mRxQueue, &eventToProccess,
                      msToWaitforVal / portTICK_PERIOD_MS)) {
      auto result = mNecParser.Parse(eventToProccess);
      if (result.has_value()) {
        auto [addr, data, isRepeat] = result.value();
        if (mDataReceivedHandler) {
...
void Transceiver::enableRx() {
  ...
  rmt_enable(mRxCh);
  xTaskCreate(this->receiveTaskImpl, RX_QUEUE_TASK_NAME, 4096, this, 5,
              &mReceiveProccess);
  mIsRxEnabled = true;
};
...
void Transceiver::send(uint16_t address, uint16_t data) {
  ...
  ESP_ERROR_CHECK(rmt_transmit(mTxCh, mNecEncoder, &scan_code,
                               sizeof(scan_code), &aTransmitConfig));
  ESP_LOGI(LOG_TAG, "Sent - address:%d data:%c)", address, data);
  // This teardown and setup feels like a bug but its the only way I could get
  // it to send properly.
  teardownTxChannel();
  setupTxChannel();
  enableTx();
