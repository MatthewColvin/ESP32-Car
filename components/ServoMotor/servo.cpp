...
uint32_t ServoMotor::angleToPulseWidth(uint32_t value) {
  return (value - ServoMotor::MIN_DEGREE) *
             (ServoMotor::MAX_PULSEWIDTH_US - ServoMotor::MIN_PULSEWIDTH_US) /
             (ServoMotor::MAX_DEGREE - ServoMotor::MIN_DEGREE) +
         ServoMotor::MIN_PULSEWIDTH_US;
}
...
void ServoMotor::attach() {
  ...
  ESP_ERROR_CHECK(
      mcpwm_generator_set_actions_on_timer_event(generator, actionEvent));
  ...
  ESP_ERROR_CHECK(
      mcpwm_generator_set_actions_on_compare_event(generator, compareAction));
...
void ServoMotor::incrementAngle(int numDegrees) {
