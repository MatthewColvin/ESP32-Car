...
public:
  ...
  /// @brief Create an instance of Mocute052 controller to manage its button
  /// presses and joystick
  /// @param aDevice - A Device found using the BTClassicHID class to scan for
  /// devices
  ...
  void onJoyStick(std::function<void(uint8_t, uint8_t)> aJoyStickHandler) {
    mHandleJoystick = std::move(aJoyStickHandler);
  };
  ...
  void onA(std::function<void()> aPressHandler,
           std::function<void()> aReleaseHandler) {
    mHandleAPress = std::move(aPressHandler);
    mHandleARelease = std::move(aReleaseHandler);
  };
  /// @brief True when a button is down false otherwise
  bool isADown() { return mIsAdown; }
  ...
  void onB(std::function<void()> aPressHandler,
           std::function<void()> aReleaseHandler) {
    mHandleBPress = std::move(aPressHandler);
    mHandleBRelease = std::move(aReleaseHandler);
  };
  /// @brief True when b button is down false otherwise
  bool isBDown() { return mIsBdown; }
  ...
  void onX(std::function<void()> aPressHandler,
           std::function<void()> aReleaseHandler) {
    mHandleXPress = std::move(aPressHandler);
    mHandleXRelease = std::move(aReleaseHandler);
  };
  /// @brief True when X button is down false otherwise
  bool isXDown() { return mIsXdown; }
  ...
  void onY(std::function<void()> aPressHandler,
           std::function<void()> aReleaseHandler) {
    mHandleYPress = std::move(aPressHandler);
    mHandleYRelease = std::move(aReleaseHandler);
  };
  /// @brief True when y button is down false otherwise
  bool isYDown() { return mIsYdown; }
  ...
  void onTrigger(std::function<void()> aPressHandler,
                 std::function<void()> aReleaseHandler) {
    mHandleTriggerPress = std::move(aPressHandler);
    mHandleTriggerRelease = std::move(aReleaseHandler);
  };
  /// @brief True when trigger is pressed down
  bool isTriggerDown() { return mIsTriggerdown; }
...
protected:
  ...
  bool handleButton(bool oldIsDown, int buttonMask, uint8_t buttonByte,
