...
class Car {
public:
  /// @brief Construct a Car that has 2 speeds cruise(set through functions) and
  /// Turbo(max speed)
  ...
  void setCruiseSpeed(float aCruiseSpeed) {
    return mMotorMixer->setCruiseSpeed(aCruiseSpeed);
  };
  ...
  /// @brief Make the car be controlled by the controller passed in
  /// @param mController
  void controlWith(std::shared_ptr<Mocute052> mController);
