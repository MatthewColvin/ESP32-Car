...
Car::Car(std::shared_ptr<Mocute052> remote, Motor *leftMotor, Motor *rightMotor)
    : mController(remote), mRightMotor(rightMotor), mLeftMotor(leftMotor),
...
void Car::mixerPollingTask() {
  while (true) {
    IMotorMixingStrategy::motorSpeeds currentSpeeds =
        mMotorMixer->getMotorSpeeds();
    // ESP_LOGI(LOG_TAG, "leftspeed:%f rightspeed:%f", currentSpeeds.left,
    // currentSpeeds.right);
...
void Car::controlWith(std::shared_ptr<Mocute052> aController) {
  mController = aController;
  setHandling(mHandling);
}
